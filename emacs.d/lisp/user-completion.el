...
The `HIPPIE-EXPAND-FUNCTION' can be generated with `make-hippie-expand-function'."
  (let ((this-command 'user/hippie-expand-completions)
        (last-command last-command)
        (buffer-modified (buffer-modified-p))
        (hippie-expand-function (or hippie-expand-function 'hippie-expand)))
    (while (progn
             (funcall hippie-expand-function nil)
             (setq last-command 'user/hippie-expand-completions)
             (not (equal he-num -1))))
...
(defun user/complete-line ()
  ...
  (interactive)
  (company-begin-backend 'company-line-backend (lambda (_line) (indent-according-to-mode))))
